...
import { Shesha } from "../core/agents";

const TaggerRouter = express.Router();

TaggerRouter.post("/", async (req: express.Request, res: express.Response) => {
  ...
  const shesha = new Shesha({
    llmApiKey: Array.isArray(llmApiKey) ? llmApiKey[0] : llmApiKey,
    ...options,
  });

  const result = await shesha.analyze(trimmedText);
